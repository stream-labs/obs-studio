...
bool obs_service_ready_to_update(obs_service_t *service)
{
	if (!obs_service_valid(service, "obs_service_ready_to_update"))
		return false;

	if (service->output)
		return obs_output_ready_to_update(service->output);
