...
struct mp_file_info media_playback_get_file_info(media_playback_t *mp)
{
	struct mp_file_info info = {0};

	if (!mp)
		return info;

	info = mp_media_get_file_info(&mp->media);

	return info;
}

bool media_playback_is_playing(media_playback_t *mp)
{
	if (!mp)
		return false;

	return mp->media.playing;
}